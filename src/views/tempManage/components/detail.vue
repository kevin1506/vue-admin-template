<template>
  <div class="lib-container">
    ...
    <div class=content>
      <div class="panel">
        <div class="head">
          ...
          <div class="opr-btns">
            <el-button type="primary" icon="el-icon-plus" @click="add">新增模板</el-button>
            <el-button @click="multSelectionOpr(1)">启用</el-button>
            <el-button @click="multSelectionOpr(0)">停用</el-button>
    ...
    <el-dialog class="opr-panel" :title="formType === 1 ? '新建模板' : '编辑模板'" @close="closeModal" :visible.sync="dialogFormVisible" width="450px">
...
<script>
import { getTempList, addTempItem, delTempItem, changeTempStatus, getTempDetail, getAllLib } from '@/api/tempManage'
  ...
  methods: {
    getList () {
      let params = {
        ...
        temmplateId: this.$parent.row.id
      }
      this.loadingStatus = true
      getTempList(params, { pageNo: this.currentPage, pageSize: this.pageSize }).then((res) => {
      ...
      getTempDetail(row.id).then((res) => {
        if (res.code === 200) {
          ...
          if (res.obj) {
            ...
            this.$nextTick(() => {
              this.form.type = res.obj.templateId
            })
          }
        } else {
          this.$message({
    ...
    deleteList (rowsArr) {
      ...
      this.$confirm('是否删除' + (names.length <= 1 ? '此条' : names.join(',')) + '?', '提示', {
        ...
        delTempItem({ ids: ids.join(',') }).then((res) => {
          if (res.code === 200) {
            this.getList()
            this.$message({
              type: 'success',
    ...
    changeStatus (ids, type) {
      ...
      changeTempStatus(params).then((res) => {
        if (res.code === 200) {
          this.getList()
          this.$message({
            message: '操作成功',
    ...
    linkToEditor (row) {
      let url = globalConfig.webUrl + '/view/temp.html?id=' + row.id + '&name=' + row.name
